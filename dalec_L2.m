...
%
% D. Aurin, NASA/GSFC January 2025
...
dataPath = '~/Projects/HyperPACE/field_data/DALEC/NF2405_VIIRS/Matlab';                    % <-- Set this
...
if rebuildL2
    ...
    fileList = dir(fullfile(L2path,'*.mat'));
    for i=1:length(fileList)
        fpf = fullfile(L2path,fileList(i).name);
        fprintf('Loading hourly L2 file: %s\n',fpf)
        load(fpf) % L2
        outFile.L2 = fileList(i).name;
        outFile.L1B = strrep(L2.ancillary(1).rawFileName,'.TXT','L1B.mat');
        %% Figures
        if makePlots
            plotL2(L2, dataPath, outFile.L2)
            pause(1)
